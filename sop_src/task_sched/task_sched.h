...
#if defined(DEBUG) && (defined(WIN32) || defined(__linux__) || defined(TARGET_OS_OSX))
...
// \func RunnableFnPtr()
// \param pCallbackData: Pointer to data defined by the application
// \param timeOrTicks: Will be "ticks" if the iterations scheduler is used,
// otherwise will be time.
// /////////////////////////////////////////////////////////////////////////////
...
extern "C" {
  ...
  */
  ...
  void TaskSchedAddTimerFn(const TaskSchedPriority prio,
    struct TaskSchedulableTag *const pSchedulable,
    const uint32_t periodMs, const uint32_t timeOffsetMs);
#else
  void _TaskSchedAddTimerFn(const TaskSchedPriority prio,
    struct TaskSchedulableTag *const pSchedulable,
    const uint32_t periodMs, const uint32_t timeOffsetMs, const char * const pFile, const int line);
...
typedef struct TaskSchedulableTag {
  ...
  // Public, function to call.  Set this parameter before passing to the
  // scheduler.
  RunnableFnPtr pTaskFn;

  // Public, data to pass to the callback function.  Set this before passing to
  // the scheduler.
  void *pUserData;
...
extern bool _TaskSchedScheduleFn(const TaskSchedPriority prio,
...
/*
  ...
  NOTE: Not thread safe, so can ONLY be called from the same context as the
  schedulable executes in.
*/
...
/*
  Initializes a schedulable with a pointer to the function to call, and the data
...
#define TS_CLASS_FN(ClassName, MemberFunction) \
  [](void *p, uint32_t t) {((ClassName *)p)->MemberFunction(t); }

// Execute a lambda function with the current timestamp.
// Note: The lambda cannot capture anything.
