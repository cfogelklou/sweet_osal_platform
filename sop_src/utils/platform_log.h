...
typedef void (*LOG_LoggingFn)(void* pUserData, const uint32_t ts, const char* szLine, const int len);
...
#ifdef __cplusplus
void LOG_HexArr(const uint8_t* const pHex, const int hexLen, const char* const pVarName = NULL);
...
#else
#define LOG_VERBOSE(x)                            \
  if ((log_TraceEnabled) && (log_VerboseEnabled)) \
    do {                                          \
      LOG_Log x;                                  \
  } while (0)
...
#define LOG_ASSERT(x)                          \
  do {                                         \
    if (!(x)) {                                \
      LOG_AssertionFailed(dbgModId, __LINE__); \
    }                                          \
  } while (0)
#if (PLATFORM_EMBEDDED > 0) // #if (PLATFORM_EMBEDDED > 0)
#define LOG_ASSERT_HPP(x)                   \
  do {                                      \
    if (!(x)) {                             \
      LOG_AssertionFailed("hpp", __LINE__); \
    }                                       \
  } while (0)
#define LOG_ASSERT_WARN_HPP(x)                     \
  do {                                             \
    if (!(x)) {                                    \
      LOG_AssertionWarningFailed("hpp", __LINE__); \
    }                                              \
  } while (0)
#else // #if (PLATFORM_EMBEDDED > 0)
#define LOG_ASSERT_HPP(x) LOG_ASSERT_FN(x)
#define LOG_ASSERT_HPP_FN(x) LOG_ASSERT_FN(x)
...
#define LOG_ASSERT_WARN(x)                            \
  do {                                                \
    if (!(x)) {                                       \
      LOG_AssertionWarningFailed(dbgModId, __LINE__); \
    }                                                 \
  } while (0)
#define LOG_ASSERT_WARN_FN(x)                         \
  do {                                                \
    if (!(x)) {                                       \
      LOG_AssertionWarningFailed(dbgModId, __LINE__); \
    }                                                 \
...
#define ASSERT_POWER_OF_TWO(sz) \
  ASSERT_AT_COMPILE_TIME(!(sz & (sz - 1)))
...
#define ASSERT_POWER_OF_TWO(pred) \
  do {                            \
  } while (0)
