...
BufIOQTransWithPayloadT* BufIOQueue_MallocWithPayload(
  ...
  if (p) {
    const BufIOQueue_TransactionCompleteCb fn =
      (cb) ? cb : _buf_ioqueue_tx_free;
...
BufIOQueue::BufIOQueue(CriticalSectionType criticalType)
  ...
  , mpEnterCritical(
      (criticalType == CriticalSectionType::TaskCritical)
...
// //////////////////////////////////////////////////////////////////////////
bool BufIOQueue::TransactionIsExpired(BufIOQTransT* const pTrans, const uint32_t timeOrTicks) {
  bool rval = false;
  if (pTrans) {
    const uint32_t time = EXPIRY_TIME_MASK & timeOrTicks;
    if (!(NO_EXPIRY_TIME_MASK & pTrans->expiryTime)) {
      const uint32_t timeDiff =
        (((uint32_t)pTrans->expiryTime - (uint32_t)time) << 1u); // MSB is now sign bit.
...
bool BufIOQueue::QueueWrite(BufIOQTransT* const pTxTrans, const uint32_t timeout) {
  if (0x87654321 != mIsValidTag)
    return false;
...
// //////////////////////////////////////////////////////////////////////////
// Queues a write to the interface, typically using an internal queue.
bool BufIOQueue::QueueRead(BufIOQTransT* const pRxTrans, const uint32_t timeout) {
  if (0x87654321 != mIsValidTag)
    return false;
...
int BufIOQueue::Write(const uint8_t* const pBytes, const int numBytes) {
  ...
  if ((pBytes) && (numBytes > 0)) {
    BufIOQTransWithPayloadT* const pTrans =
      BufIOQueue_MallocWithPayload(numBytes, pBytes);
    if (pTrans) {
...
int BufIOQueue::GetWriteReady() const {
  return 2048;
}
...
int BufIOQueue::GetReadReady() const {
  return 0;
}
...
// //////////////////////////////////////////////////////////////////////////
// Go through all old messages and purge any which have a
// time older than the expiry time.
...
// //////////////////////////////////////////////////////////////////////////
BufIOQTransT*
  BufIOQueue::LoadQueuedTxTransaction(const uint8_t*& pTxBuf, int& len) {
  return LoadNextTransaction(TX, (uint8_t**)&pTxBuf, len);
}

// //////////////////////////////////////////////////////////////////////////
BufIOQTransT*
  BufIOQueue::LoadQueuedRxTransaction(uint8_t*& pRxBuf, int& len) {
  return LoadNextTransaction(RX, &pRxBuf, len);
}

// //////////////////////////////////////////////////////////////////////////
bool BufIOQueue::CommitTxTransaction(const int len, const bool callCallbackIfCompleted) {
  return CommitTransaction(TX, len, callCallbackIfCompleted);
}

// //////////////////////////////////////////////////////////////////////////
bool BufIOQueue::CommitRxTransaction(
  const int len,
...
// START private
// //////////////////////////////////////////////////////////////////////////
bool BufIOQueue::Queue(const Dir dir, BufIOQTransT* const pTrans, const uint32_t timeout) {
  if (0x87654321 != mIsValidTag)
    return false;
  bool rval = false;
  ...
  if (pTrans && pTrans->pBuf8 && pTrans->transactionLen > 0) {
    pTrans->expiryTime =
      ((0 == timeout) || (OSAL_WAIT_INFINITE == timeout))
        ? OSAL_WAIT_INFINITE
...
// //////////////////////////////////////////////////////////////////////////
bool BufIOQueue::CommitTransaction(
  const Dir dir, const int len,
  const bool callCallbackIfCompleted, const bool forceCompleted) {
  if (0x87654321 != mIsValidTag)
    return false;
  ...
  if (NULL == pCurr) {
    ...
    if (!((forceCompleted) ||
          (pCurr->transferredIdx >= pCurr->transactionLen))) {
      mpExitCritical();
    } else {
...
// //////////////////////////////////////////////////////////////////////////
BufIOQTransT*
  BufIOQueue::LoadNextTransaction(const Dir dir, uint8_t** ppBuf, int& len) {
