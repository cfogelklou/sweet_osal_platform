...
#if defined(CC26XXWARE) || defined(ANDROID) || defined(__EMSCRIPTEN__)// || defined(TARGET_OS_IOS)
...
#define OSAL_INSTANTIATE_STACK(name, size, taskId)    \
  ...
  static const OSALTaskStructT name =   \
    { name##_stack, (size), taskId }
...
// For use by test case code only!
#define OSAL_FREE_STACK(name) \
  OSALFREE(name##_stack)
