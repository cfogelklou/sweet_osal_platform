...
static void    sp_ForEachGridLine(SP_PlotT *pPlot, SPAxisPickerT nAxis, SPGridPickerT nGrid, pFnForEachGridLineCallback pCB, void *pUserData);
static void    sp_DrawGridlinesCallback(SP_PlotT *pPlot, SPAxisPickerT nAxis, SPGridPickerT nGrid, SP_FloatT fGridValue, void *pUserData);
static void    sp_DrawGridlines(SP_PlotT *pPlot);
...
SP_ColourT SP_SetBackgroundColour(SP_PlotT *pPlot, SP_ColourT nColour)
{
...
void SP_CopyAxis(SP_PlotT *pPlot, SPAxisPickerT nAxis, SP_AxisT *pSrcAxis)
{
   ...
   SP_SetAxisScaleType( pPlot, nAxis, pDestAxis->nScaleType );
...
//---------------------------------------------
void    SP_SetGridUnits(SP_PlotT *pPlot, SPAxisPickerT nAxis, SP_FloatT fMajorUnits, SP_FloatT fMinorUnits)
{
    SP_AxisT *pDestAxis = (nAxis == SP_XAXIS) ? &pPlot->XAxis : &pPlot->YAxis;
...
void SP_RemoveWaveform(SP_PlotT *pPlot, SP_IntT nID)
{
    ...
    while (!nDone && (i < SP_MAX_DATASETS))
    {
        ...
        if ((pNewDataSet->pDS) && (pNewDataSet->pDS->nID == nID))
        {
            ...
            pPlot->DataSets[pPlot->nDataSets].pDS = NULL;
        }
        else
        {
            i++;
...
void SP_ReDraw(SP_PlotT *pPlot)
{
    ...
    // Draw the plot data
    SP_ForEachDataSet(pPlot, sp_ReDrawCallback, (void *)pPlot);
}
...
SP_IntT SP_Init(SP_PlotT *pPlot, SP_GfxApiT *pGP, SP_ColourT nBackgroundColour)
{
    ...
    if (!(pPlot)) return SPLOT_ERROR;
    if (!(pGP)) return SPLOT_ERROR;

    memset(pPlot, 0, sizeof(SP_PlotT));

    // Initialize dataset IDs to -1.
    for (i = 0; i < SP_MAX_DATASETS; i++)
    {
        SP_DataSetPlotT *pDS = &pPlot->DataSets[i];
        pDS->pDS = NULL;    // -1 indicates an invalid dataset.
    }
...
static void sp_ReDrawCallback(SP_DataSetPlotT *pDataSetPlot, void *pUserData)
{
    ...
    SP_PlotT                *pPlot      = (SP_PlotT *)pUserData;
    SP_GfxApiT              *pGP        = pPlot->pGP;
    SP_DataSetT             *pDS        = pDataSetPlot->pDS;
    SP_IntT                 nXAxisPixelRange = pGP->nWidth-1;
    SP_IntT                 nYAxisPixelRange = pGP->nHeight-1;

    SP_FloatT               fMinX = (pDS->fMinX < pPlot->XAxis.fMin) ? pPlot->XAxis.fMin : pDS->fMinX;
    SP_FloatT               fMaxX = (pDS->fMaxX > pPlot->XAxis.fMax) ? pPlot->XAxis.fMax : pDS->fMaxX;
